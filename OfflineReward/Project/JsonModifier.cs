...
{
    public static string AddToFieldValues(string json, Dictionary<string, object> updates, ILogger<RewardController> logger)
    {
        ...
        if (updates.ContainsKey("Exp"))
        {
            ...
            while (true)
            {
                string formula = RewardController.levelUpRequireExp.Replace("{level}", currentLevel.ToString());
