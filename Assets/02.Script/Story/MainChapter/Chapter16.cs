...
public class Chapter16 : StoryChapter
{
    ...
    public override void BuildActions(StoryRunner runner)
    {
        ...
        runner.AddAction(runner.Dialogue("하지만 결정했다."));
        runner.AddAction(runner.Dialogue("너를 돌려보내지 않는다."));
        runner.AddAction(runner.Dialogue("너를 이 세계의 안에 둔다."));
        ...
        runner.AddAction(runner.SetTalkerAction(player));
        runner.AddAction(runner.Dialogue("으으으음."));
        runner.AddAction(runner.Dialogue("나 한 사람 돌려보겠다고 더 큰일 나는건 싫어."));
        runner.AddAction(runner.Dialogue("그리고 난 이 세계도 맘에 드는걸."));

        runner.AddAction(runner.SetTalkerAction(archer));
        runner.AddAction(runner.Dialogue("그래서 남겠다는 거네."));

        runner.AddAction(runner.SetTalkerAction(warrior));
        runner.AddAction(runner.Dialogue("돌아간다고 했으면 아쉬웠을거다!"));

        runner.AddAction(runner.SetTalkerAction(mage));
        runner.AddAction(runner.Dialogue("큰 붕괴는 막았지만 이미 꼬인 부분들이 남아 있어."));
        ...
        runner.AddAction(runner.SetTalkerAction(warrior));
        runner.AddAction(runner.Dialogue("좋지. 심심할 틈은 없겠다."));
