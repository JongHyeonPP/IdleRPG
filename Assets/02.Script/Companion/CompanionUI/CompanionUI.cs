...
public class CompanionUI : MonoBehaviour, IMenuUI
{
    ...
    private VisualElement[] _companionLockPanelArr;  // 동료 잠금 오버레이
    ...
    private void InitPanelButton()
    {
        ...
        SetPanel(0); // 초기 표시 (소리 없음)
        _currentPanelIndex = 0;
    }

    private void OnClickPanelButton(int currentIndex)
    {
        if (_currentPanelIndex != currentIndex)
            SoundManager.instance.PlaySFX(SoundPath.BtnClick2);

        _currentPanelIndex = currentIndex;
        SetPanel(currentIndex);
    }

    private void SetPanel(int currentIndex)
    {
        for (int i = 0; i < _panelArr.Length; i++)
        {
            bool isActive = i == currentIndex;
            _panelArr[i].style.display = isActive ? DisplayStyle.Flex : DisplayStyle.None;

            var btn = _panelButtonArr[i];
    ...
    private void InitTechButton()
    {
        ...
        _currentTechIndex = 0;
        OnTechButtonClick(0); // 초기 표시 (소리 없음)
    }
    ...
    private void OnTechButtonClick(int techCompanionIndex)
    {
        if (!IsCompanionUnlocked(techCompanionIndex))
        {
            ...
            UIBroker.ShowPopUpInBattle($"스테이지 {required} 도달 시 해금됩니다.");
            return;
        }
        ...
        _currentTechIndex = techCompanionIndex;
        ...
        for (int i = 0; i < _techButtonArr.Length; i++)
        {
            ...
            {
                _techButtonArr[i].style.unityBackgroundImageTintColor =
                    new Color(activeColor.r, activeColor.g, activeColor.b, 0.1f);
            ...
            {
                _techButtonArr[i].style.unityBackgroundImageTintColor =
                    new Color(inactiveColor.r, inactiveColor.g, inactiveColor.b, 0f);
    ...
    private void SetCurrentTechData(int techIndex_0, int techIndex_1)
    {
        ...
        CompanionTechData companionTechData =
            CompanionManager.instance.GetCompanionTechData(currentTechCompanionIndex, techIndex_0, techIndex_1);
    ...
    {
        VisualElement techClickPanel = _panelArr[1].Q<VisualElement>($"TechClickPanel_{techIndex_0}_{techIndex_1}");
        techClickPanel.Q<Button>("ClickButton")
            .RegisterCallback<ClickEvent>(evt => OnClickTechVe(techIndex_0, techIndex_1));
    }
